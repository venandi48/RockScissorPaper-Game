...
machineChoices.forEach((element) => {
  element.querySelector("input[type=hidden]").value = Random(0, 2);
});

controlBtns.forEach((controlBtn) => {
  replayBtn = controlBtn.querySelector("input[type=button][value=Replay]");
  replayBtn?.addEventListener("click", (e) => {
    replay(controlBtn.parentElement.parentElement);
  });
});
    ...
    machineChoices.forEach((element) => {
      if (element.classList.contains("selected") === false) {
        ...
        element.querySelector("input[type=hidden]").value =
          (element.querySelector("input[type=hidden]").value + 1) % 3;
...
function replay(playbox) {
  if (playbox.querySelector(".machine").classList.contains(".selected"))
    playbox.querySelector(".machine").classList.remove("selected");
  resetResultOfCss(playbox.querySelector(".machine .choice"));

  playbox
    .querySelector(".user")
    .querySelectorAll(".choice")
    .forEach((choiceElement) => {
      resetResultOfCss(choiceElement);
    });
}
